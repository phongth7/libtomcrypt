...
\hline LTC\_ASN1\_CUSTOM\_TYPE       & A custom type (see LTC\_SET\_ASN1\_IDENTIFIER) \\
\hline
\end{tabular}
...
\index{LTC\_SET\_ASN1\_IDENTIFIER macro} \index{LTC\_ASN1\_CUSTOM\_TYPE}

To be able to encode and decode all other valid ASN.1 Identifiers, such as Context-Specific types the macro
\textit{LTC\_SET\_ASN1\_IDENTIFIER(list, index, Class, Pc, Tag)} has been provided.

It will assign to the \textit{index}th position in the \textit{list} the Identifier-triplet (Class, Pc, Tag).
It has to be used in combination with the \textit{LTC\_SET\_ASN1} macro.  An example usage would be:

\begin{small}
\begin{verbatim}
...
ltc_asn1_list   sequence[3], custom[1];
unsigned long   three=3;
...
LTC_SET_ASN1(custom, 0, LTC_ASN1_CUSTOM_TYPE,   sequence,  1);
LTC_SET_ASN1_IDENTIFIER(custom, 0, LTC_ASN1_CL_CONTEXT_SPECIFIC, LTC_ASN1_PC_CONSTRUCTED, 0);
\end{verbatim}
\end{small}

This would (un)pack the given sequence from/in the context-specific tag \textit{[0]}.
...
To be able to represent all other valid types besides the primitive types having their own decoder, the \textit{custom_type} de- & encoders are provided.
...
The usage of this de- & encoder is a bit different than the others since the type to be encoded has to be passed to the function.
...
and \textit{LTC\_SET\_ASN1\_IDENTIFIER} macros.
