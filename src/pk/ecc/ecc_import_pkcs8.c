                                ...
                                unsigned char *dec_data, unsigned long *dec_size)
{
   ...
   if (id == PBE_SHA1_DES || id == PBE_SHA1_RC2) hid = find_hash("sha1");

   if (id == PBE_MD2_DES || id == PBE_MD5_DES || id == PBE_SHA1_DES) cid = find_cipher("des");
   if (id == PBE_MD2_RC2 || id == PBE_MD5_RC2 || id == PBE_SHA1_RC2) cid = find_cipher("rc2");

   if (hid != -1 && cid != -1) {
      if (salt_size != 8 || enc_size < 8) goto LBL_ERROR;
      if ((err = pkcs_5_alg1(pass, pass_size, salt, iterations, hid, k, &klen)) != CRYPT_OK) goto LBL_ERROR;
      if ((err = cbc_start(cid, k + 8, k, 8, 0, &cbc)) != CRYPT_OK) goto LBL_ERROR;
